...
try:
    from recruitR.library.debug import DEBUG
except ImportError:
    DEBUG = False


try:
    from recruitR.library.debug import DEBUG_STORAGE
except ImportError:
    DEBUG_STORAGE = False


try:
    from recruitR.library.debug import PROXY
except ImportError:
    PROXY = ''
...
class APIHTTP:
    ...
    def send_api_request(self, endpoint, parameters, referer=None, proxy=None, headers=None, timeout=None, raise_exception_on_error=False):
        ...
        data = self.api_response(response=contents, status_code=status_code, url=url)
