...
def tfs_transfer_portal_player_rankings(sport_key: int, year: int, page_size = 500, headers = None):
    ...
    url = TFS_BASE_URL + "rankings/{}/{}/transferPortalPlayerFeed".format(sport_key, year)
    response = requests.get(url, params=params, headers = headers)
    resp_json = response.json()
    resp = pd.json_normalize(resp_json['rankings'] )
